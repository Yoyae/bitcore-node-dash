...
To develop bitcore-node-monoeci:

```bash
cd ~
git clone git@github.com:<yourusername>/bitcore-node-monoeci.git
...
We now will setup symlinks in `bitcore-node-monoeci` *(repeat this for any other modules you're planning on developing)*:
```bash
cd node_modules
rm -rf bitcore-lib
ln -s ~/bitcore-lib
rm -rf bitcoind-rpc-monoeci
ln -s ~/bitcoind-rpc-monoeci
```
...
cd devnode
mkdir node_modules
touch bitcore-node-monoeci.json
touch package.json
```

Edit `bitcore-node-monoeci.json` with something similar to:
```json
{
  "network": "livenet",
  "port": 3001,
...
```bash
../bitcore-node-monoeci/bin/bitcore-node-monoeci start
```